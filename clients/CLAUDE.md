...
- `llm_provider.py` — Universal LLM entry point. Owns streaming events, tool execution with circuit breaker, prompt caching, billing integration, and Anthropic failover. SIGUSR1 toggles the LLM traffic tap (`utils/llm_tap.py`); response logging at 4 sites (Anthropic stream/non-stream, generic stream/non-stream). Exports `ThinkingConfig`, `EFFORT_BUDGET_MAP`, `build_batch_params`, `ToolCall`, `ToolExecution`, `ToolExecutionResult`.
