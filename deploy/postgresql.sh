...
if [ "$CONFIG_OFFLINE_MODE" = "yes" ]; then
    ...
    OFFLINE_SQL="INSERT INTO account_tiers (name, model, thinking_budget, description, display_order, provider, endpoint_url, api_key_name, hidden) VALUES ('offline', '${OLLAMA_MAIN}', 0, 'Local (Ollama)', 0, 'generic', 'http://localhost:11434/v1/chat/completions', NULL, FALSE) ON CONFLICT (name) DO UPDATE SET model = '${OLLAMA_MAIN}', endpoint_url = 'http://localhost:11434/v1/chat/completions', api_key_name = NULL; UPDATE users SET llm_tier = 'offline'; UPDATE internal_llm SET endpoint_url = 'http://localhost:11434/v1/chat/completions', model = '${OLLAMA_MAIN}', api_key_name = NULL WHERE endpoint_url LIKE 'https://%' AND name != 'analysis'; UPDATE internal_llm SET endpoint_url = 'http://localhost:11434/v1/chat/completions', model = '${OLLAMA_SUB}', api_key_name = NULL WHERE name = 'analysis';"
    if [ "$OS" = "linux" ]; then
