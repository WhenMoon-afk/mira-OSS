...
- `orchestrator.py` — Owns the message→LLM→response cycle: memory surfacing, system prompt composition, tool execution loop, context overflow remediation (embedding-based drift pruning then oldest-first fallback), and tool/thinking result persistence. Singleton via `get_orchestrator()`. Exports `TurnMetadata`, `LLMKwargs`, `ToolInteraction`.
