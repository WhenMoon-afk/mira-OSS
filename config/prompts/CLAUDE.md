...
- `agents/email_sidebar_system.txt` — Email agent base prompt: always loaded for system-level orientation (workflow mechanics, tool batching, escalation guardrails). Per-rule prompts are appended, not substituted. Consumer: `agents/implementations/email_sidebar.py`.
