...
Implementations define: `agent_id`, `internal_llm_key`, `available_tools`, `get_agent_prompt(work_item)`, `build_initial_message()`. Override `on_completion()` to publish to a different trinket (see `ForageAgent`). Default agent prompts live in `config/prompts/agents/` as `.txt` files, loaded via `load_agent_prompt()` from `base.py`. Per-rule prompts are **additive** — the base prompt (`email_sidebar_system.txt`) always loads for system-level orientation (workflow mechanics, tool batching, escalation guardrails), and per-rule prompts from `work_item.context['agent_prompt']` are appended inside `<rule_instructions>` tags. If no per-rule prompt is configured, the agent escalates by default. If multiple rules with different prompts match the same item, the trigger writes a `conflict` record to `sidebar_activity` and skips the item.
...
- `implementations/email_sidebar.py` — `EmailSidebarAgent`: handles emails. Always loads `email_sidebar_system.txt` as base prompt (workflow, tool batching, escalation). Per-rule prompt appended in `<rule_instructions>` tags; without one, agent escalates all emails by default. Sets `sanitize_untrusted_input=True` for injection defense. `max_retries=1` with `build_recovery_context()` override. Tools: `email_tool`.
